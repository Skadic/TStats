...
redis:
  docker run --rm --name tstats-redis -p 6379:6379 -d redis

# Run the dragonfly container
dragonfly:
  docker run --network=host --rm --name tstats-redis -p 6379:6379 --ulimit memlock=-1 -d docker.dragonflydb.io/dragonflydb/dragonfly

# Run the postgres container
postgres:
  docker run --rm --name tstats-postgres -p 5432:5432 -e POSTGRES_USER=root -e POSTGRES_PASSWORD=root -d postgres:17
...
# Runs caddy
caddy:
  docker run --rm -v caddy_data:/data:z -v $PWD/Caddyfile:/etc/caddy/Caddyfile:z --name tstats-caddy -p 9900:9900 -p 9901:9901 caddy:latest

# Builds all the web-app containers and runs them
compose:
  docker-compose up --build -t 0 --force-recreate
